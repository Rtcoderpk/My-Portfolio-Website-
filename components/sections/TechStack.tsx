...
import { Brain, Layers3, Boxes, Database, Code2, Server, Container, Cloud, ChevronRight } from 'lucide-react';
        ...
        <div className="grid lg:grid-cols-[0.7fr_1.3fr] gap-14 items-center">
          ...
          >
            ...
            <p className="text-gray-300 text-base leading-relaxed mb-8 max-w-md">
              An ecosystem of modern tools and technologies working together to build
              powerful AI applications.
            </p>
            <button type="button" className="btn-secondary">
              Explore Stack <ChevronRight size={16} />
            </button>
          </motion.div>
          ...
          {isMobile ? (
            ...
            <div className="relative w-full max-w-[340px] sm:max-w-[460px] lg:max-w-[600px] aspect-square mx-auto">
              {/* Center hub — symmetric radial glow, no outer dashed rings */}
              <motion.div
              ...
              >
                ...
                <span className="mt-2 text-[10px] sm:text-xs font-mono tracking-[0.15em] uppercase text-violet-glow/80 whitespace-nowrap">
                  Core AI Expertise
                </span>
              </motion.div>

              {/* Nodes — same radius for all, core cards visually distinct */}
              {points.map((node, i) => (
                <motion.div
                  key={node.title}
                  ...
                  className={`absolute z-10 flex flex-col items-center text-center gap-1.5 cursor-pointer
                    ${node.core
                ...
                >
                  ...
                  `}>
                    <node.icon
                      size={13}
                      className={`sm:w-[15px] sm:h-[15px] ${node.core ? 'text-violet-glow' : 'text-secondary'}`}
                    />
                  </div>
                  <div className="min-w-0 flex flex-col items-center min-h-[2.8rem] sm:min-h-[3.2rem] justify-center">
                    <div className="flex items-center gap-1.5 flex-wrap justify-center">
                      <span className={`font-semibold leading-tight ${node.core ? 'text-[11px] sm:text-xs text-white' : 'text-[11px] sm:text-xs text-white'}`}>
                        {node.title}
                      </span>
                      {node.badge && (
                        <span className="text-[8px] sm:text-[9px] font-mono text-violet-glow/70 bg-violet-glow/10 px-1.5 py-[1px] rounded leading-tight">
                          {node.badge}
                        </span>
                      )}
                    </div>
                    <div className="hidden sm:block text-[10px] lg:text-[11px] text-gray-300 leading-tight mt-0.5">
                      {node.sub}
                    </div>
                  </div>
                </motion.div>
